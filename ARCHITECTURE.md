...
└── carApp/                               # Expo app root
    ...
    │       │   ├── index.tsx 
    │       │   ├── past.tsx 
    │       │   ├── [id].tsx                # Customer booking detail + cancel; entry point to the quote approval screen 
    │       │   ├── quote/[bookingId].tsx   # Customer reviews the provider's quote → acceptQuote + deposit PaymentSheet 
    ...
    │       │   ├── quote/[bookingId].tsx  # Provider prices an unpriced request → submit_quote 
    ...
    │   │   ├── booking/                  # DateTimePicker, ArrivalWindowPicker, AddressPicker, PriceBreakdown, DepositSummary, StatusTimeline, ReviewSheet, BookingPhotoGallery, VehicleConditionForm 
    │   │   ├── tracking/                 # LiveMap, JobStatusBar, ETADisplay 
    │   │   ├── provider/                 # CredentialUpload, AvailabilityCalendar, VettingStepIndicator, VettingUploadStep, VettingActionStep, ServiceMenuEditor, EarningsDashboard, JobPhotoCapture, DayTimeline, QuoteBuilder 
    ...
    │       ├── _shared/                   # fcm.ts, email.ts (Resend), webhookSignature.ts (HMAC verify), quote.ts (quote grammar + totals), shared helpers 
    │       ├── stripe-webhook/            # Payment engine (app-invoked, verify_jwt: true): intents, capture, refunds, accept/decline, submit_quote, cancel, no-show, Connect, payouts
    │       ├── stripe-events/             # Stripe webhook receiver (verify_jwt: false, Stripe-Signature auth): payment_intent.succeeded/failed 
...
- **Booking write surface (server-authoritative)**: RLS has no column-level granularity, so the client's vocabulary on `bookings` is fixed by **column privileges** plus SECURITY INVOKER triggers. INSERT allows `id, customer_id, provider_id, vehicle_id, package_id, services, status, scheduled_at, service_address, location_lat, location_lng, notes`; UPDATE allows `scheduled_at, status, started_at`, and `enforce_booking_status_transition` narrows that to customer `pending → cancelled` and provider `confirmed → en_route → in_progress`. **The client cannot state a price or a duration at any point** — `derive_booking_amounts` recomputes every money column from `service_packages` and rebuilds the services snapshot from the same rows. The triggers must stay SECURITY INVOKER: as DEFINER, `current_user` always reads `'postgres'`, so their trusted-role early return would match every write and silently disable the guard. Edge Functions are unaffected (service role).
- **Provider profile write surface**: `"provider_profiles: write own"` is `FOR ALL USING (auth.uid() = user_id)`, which scopes writes to the caller's own row correctly — but RLS has no column-level granularity, so column privileges do that half. The client may INSERT `id, user_id, provider_type_id` and UPDATE `bio, coverage_area, mile_radius, base_lat, base_lng, availability, default_buffer_before_mins, default_buffer_after_mins`. Everything else is **service-role only**: `platform_fee_rate` and the founding-provider fields (fee tier), `verification_status`/`approved_at` (the vetting gate — `admin-review-provider` owns these), `stripe_account_id` (payout routing, written by Connect onboarding), and `avg_gear_rating`/`total_jobs`/`kudos_count` (reputation, which drives search ranking). DELETE is revoked outright — the row is the FK target for bookings, payouts and service packages, so account deletion is a service-role operation. `provider_type_id` is INSERT-only: a type switch should re-enter vetting, since the six steps are type-specific.
- **Provider calendar (working hours, timezone, time off)**: `provider_profiles.working_hours` holds per-day local windows — `{"mon":[{"start":"08:00","end":"18:00"}],"sat":[]}` — where an empty or missing day means closed and an array allows a split day (mornings and evenings, or a lunch break). Times are **wall-clock strings, never instants**: `provider_profiles.timezone` (IANA, default `America/New_York`) is what turns them into real times, and storing an offset instead would shift every window twice a year at the DST boundary. `trg_validate_provider_schedule` rejects an unknown zone or a malformed window, so every reader can trust the shape. The legacy `availability` boolean map is kept for older clients; `workingHoursFromJson()` and `availabilityFromJson()` both read **either** shape, and a legacy `true` maps to the default 08:00–18:00 window — the same substitution the migration backfill makes, so the two can never disagree. `provider_time_off` holds one-off blocks with a generated `blocked_range` and an EXCLUDE constraint against overlapping blocks. **None of this is enforced against bookings**: working hours are a stated preference, not an invariant, so `DayTimeline` surfaces the clash and the provider decides. The EXCLUDE constraint stays reserved for what is genuinely impossible (two jobs at once). All positioning maths lives in `src/utils/schedule.ts` and projects into the *provider's* local day, never the device's.
- **Booking occupancy & the overlap guard**: A booking occupies `occupied_range`, a generated `tstzrange` over `[scheduled_at − buffer_before, scheduled_at + estimated_duration_mins + buffer_after)`. Buffers are snapshotted onto the row from `provider_profiles.default_buffer_before_mins/after_mins` at insert, so retuning a provider's defaults never rewrites jobs already on the calendar; travel time lives in the *after* buffer (no routing API in MVP). The `bookings_no_provider_overlap` EXCLUDE constraint (`btree_gist`) makes it impossible for one provider to hold two overlapping bookings in `confirmed`/`en_route`/`in_progress`. `pending` and `pending_provider_approval` are deliberately outside the guard — several customers may request the same window and the **first accept wins**, which is where `23P01` fires. The client surfaces it as *"That time was just taken"* (`SlotUnavailableError`); the provider's accept gets a 409 with `code: 'slot_conflict'`. Note the range keys off `scheduled_at`, **not** `started_at` — unlike `estimated_completion_at`, which follows reality so the customer's ready-by time reflects a late start. Keying occupancy off `started_at` would let a late start slide the range into the next job and 23P01 the provider out of tapping Start Job; overruns are handled by notification, not by the constraint.
- **Deposit model**: 15% of booking total collected at booking via Stripe; remainder captured on job completion.
- **Quote-first booking is the booking flow.** `search/book/[providerId].tsx` collects **no payment**. It inserts a `pending_provider_quote` row carrying `requested_window_start`/`requested_window_end` and stops; the provider prices it through `submit_quote`, and the customer pays only after approving on `bookings/quote/[bookingId].tsx` via `accept_quote` + the existing `create_deposit_intent` → PaymentSheet pair. The deposit-first path remains valid for rows already in flight — `stripe-events` distinguishes the two by `quoted_total_amount` being non-null, which is NULL on every deposit-first booking ever made. Three consequences worth knowing: `scheduled_at` is NOT NULL, so an unpriced request stands it up from the window start until `submit_quote` overwrites it; the overlap `EXCLUDE` constraint covers only `confirmed`/`en_route`/`in_progress`, so an unpriced request holds no slot and two customers may request the same window; and the prices shown before a quote are the provider's **advertised** rates, an estimate, never what the customer is charged.

- **The app imports validation bounds from `supabase/functions/_shared/quote.ts`.** `QuoteBuilder` takes `MIN_QUOTE_DURATION_MINS`, `MAX_QUOTE_DURATION_MINS`, `MAX_QUOTE_LINE_ITEMS` and `MAX_LINE_ITEM_LABEL_LENGTH` from the Edge Function's own module rather than restating them, so a control cannot offer a value `prepareQuote` will reject. This is the one place app code reaches into `supabase/functions/`, and it works because `_shared/quote.ts` carries no remote imports — which is also why `tsconfig.json` leaves it outside the exclude list and Jest can test the shipping code directly. Keep it that way: adding a Deno or `https://` import to that module would break both the app build and its tests.

- **Stripe webhook auth split**: Stripe deliveries land on **`stripe-events`**, not `stripe-webhook`. The two callers need opposite auth models: app invocations carry a Supabase JWT and rely on `verify_jwt: true`, while Stripe cannot attach one, so a JWT-verified endpoint rejects every delivery with 401 before the handler runs. `stripe-events` therefore runs with `verify_jwt = false` and authenticates by verifying `Stripe-Signature` against `STRIPE_WEBHOOK_SECRET`, refusing any request without a signature. Both Stripe functions declare their flag in `carApp/supabase/config.toml` (`[functions.stripe-webhook] verify_jwt = true`, `[functions.stripe-events] verify_jwt = false`) rather than relying on `--no-verify-jwt` being passed at deploy time; the flag is a property of the endpoint's auth model, so leaving it to a remembered command-line argument means one forgetful redeploy silently 401s every live Stripe delivery. `stripe-webhook` keeps `verify_jwt: true` and holds no webhook code — important, because most of its actions perform service-role writes with no in-code caller authentication of their own: `verify_jwt` proves *a* valid user called, never *which* one, so `accept_booking` and its siblings are reachable with any authenticated user's token. `submit_quote` is the first action to close that gap for itself, resolving the caller via `auth.getUser(token)` and refusing anyone who does not own the booking's provider profile (the `update-provider-location` pattern) — necessary because it sets a price, which is the one thing the booking write surface exists to keep away from clients. The older actions still need the same treatment. Deposit → `pending_provider_approval` and the `payments.status` transitions only ever happen in `stripe-events`; the client never asserts paid state.
- **Deposit checkout (PaymentSheet)**: The card is collected in Stripe's native **PaymentSheet**, not a `CardField` — the app renders no card inputs of its own. `create_deposit_intent` returns the client secret plus the Stripe `customerId` and an ephemeral key; the client passes the customer pair only when the server sends both, so the sheet still opens against older function versions (it just loses the saved-card list). The PaymentIntent carries `customer` + `setup_future_usage: 'off_session'` server-side, which is what lets `capture_balance` charge the saved card later without the customer present. **Booking rows are created before payment** because the Edge Function needs a `booking_id` to attach the intent to — so the booking screen cancels the row if the intent fails, the card is declined, or the customer dismisses the sheet. Nothing but a successful `payment_intent.succeeded` webhook moves a booking to `pending_provider_approval`.
