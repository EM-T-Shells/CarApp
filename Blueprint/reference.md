...
[carApp/src/lib/supabase/auth.ts] — signInWithOtp/verifyOtp narrow OtpTarget with `target.email !== undefined` instead of truthiness. An empty-string email is falsy but still matches the email union member, so the truthy check never narrowed and left phone possibly-undefined against Supabase's credential types.
