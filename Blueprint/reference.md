...
[carApp/src/state/mode.ts] — Persisted (AsyncStorage) Zustand store holding `activeMode` (customer|provider) for dual-role users' dashboard switch. Only consulted for role 'both'; read by the auth gate + both More hubs, set only by the two switch controls.
[carApp/app/_layout.tsx] — useProtectedRoute now mounts (tabs) vs (provider-tabs) by role + activeMode: customer→(tabs), approved provider→(provider-tabs), 'both'→provider-tabs only when activeMode='provider' & approved (waits on mode rehydration to avoid a flash).
[carApp/app/(provider-tabs)/] — New provider dashboard tab group (Jobs/Inbox/Earnings/More). jobs/index + jobs/past extracted from the old customer bookings provider-toggle branch; jobs/[bookingId] moved from (tabs)/bookings/job; earnings + more/manage moved from (tabs)/more/provider-earnings + provider-manage; inbox/index is new (getThreadsForProvider → shared (tabs)/inbox/[threadId]); more/index hosts "Switch to Customer Dashboard" (dual-role only).
...
[carApp/supabase/migrations — recategorize_addon_services_by_provider_type] — Retired the 'addon' service_catalog category: detailer add-ons → 'detailing', mechanic add-ons → 'mechanical' (each row already carried a provider_type_id). schema.sql seed + seeds/service_catalog.sql updated to match. Services tab now shows two sections.
[carApp/src/lib/supabase/queries.ts] — getProvidersByService(catalogId): approved providers with an active, approved service_packages row for a catalog id (service_packages!inner join). Returns ProviderSearchResult so results render with ProviderCard; sorted by rating (no origin → no distance sort).
[carApp/app/(tabs)/services/[catalogId].tsx] — Provider list for a single catalog service, reached by tapping a service in the catalog. Fetches getProvidersByService, renders ProviderCard tiles, taps through to /search/provider/[id]. Handles loading/empty/error.
[carApp/app/(tabs)/services/index.tsx] — Catalog cards are now pressable (navigate to /services/[catalogId] with the service name); dropped the 'Add-ons' category label now that the category is retired.
[carApp/src/components/search/ProvidersMap.tsx] — Map view of search results (Turo-style list⇄map toggle). Drops an OSM pin (no API key, same basemap as LiveMap) per provider with base_lat/base_lng + the customer's origin, fits the camera to all points, and floats a ProviderCard preview when a pin is tapped (tap card → open profile).
[carApp/src/lib/location/index.ts] — Added regionForCoordinates(points[]) + MapRegion type: bounding-box region fitting N points (padded, min-delta floor) used to frame every provider pin on the results map. Returns null for an empty list so callers fall back to a default region.
[carApp/app/(tabs)/search/results.tsx] — Added a list/map view toggle: a floating Map/List pill (shown once there are results) flips a viewMode state that swaps the FlatList for <ProvidersMap>, wired to the store's results + origin.
[carApp/src/lib/stripe/index.ts] — Replaced confirmDepositPayment() with presentDepositPaymentSheet(): opens Stripe's PaymentSheet (initPaymentSheet + presentPaymentSheet) against the deposit client secret. The old call assumed a mounted CardField that never existed anywhere in the app, so every deposit failed with "Card details not complete". Passes customerId/ephemeralKeySecret only when the Edge Function supplies both (saved-card list), and reports a dismissed sheet as { canceled: true } with no error so callers can unwind quietly.
[carApp/supabase/functions/stripe-webhook/index.ts] — create_deposit_intent now mints a Stripe ephemeral key (pinned to STRIPE_API_VERSION) and returns customerId + ephemeralKeySecret alongside the client secret, so PaymentSheet can show the customer's saved cards.
[carApp/app/(tabs)/search/book/[providerId].tsx] — Review step now pays via presentDepositPaymentSheet and cancels the booking row when the intent fails, the card is declined, or the customer dismisses the sheet — previously a failed payment left a 'pending' booking showing as scheduled in the Bookings tab.
[carApp/supabase/functions/stripe-events/index.ts] — NEW. Stripe webhook receiver, split out of stripe-webhook because the two callers need opposite auth: app invocations rely on verify_jwt:true, but Stripe cannot send a Supabase JWT, so every delivery was being 401'd by the platform before the handler ran (no real payment_intent.succeeded had ever processed). Deployed --no-verify-jwt and authenticated by Stripe-Signature; refuses requests without one. Owns onPaymentIntentSucceeded (payments→succeeded, booking→pending_provider_approval + notify) and onPaymentIntentFailed. Returns 500 on handler error so Stripe retries.
[carApp/supabase/functions/stripe-webhook/index.ts] — Removed the Stripe-Signature branch and both event handlers; now app-actions only and stays verify_jwt:true (it does service-role writes with no in-code caller auth, so it must not be public). APPROVAL_WINDOW_MS moved to stripe-events, which stamps approval_expires_at; the expire sweep here reads that column directly.
[carApp/src/design/tokens.ts] — Added the `Palette` type (alias of ColorMap) for values holding either colors.light or colors.dark. Screens had typed their palette prop as `typeof colors.light`, which pins the light-mode hex literals via `as const` and rejects the dark palette, so every dark-mode call site was a type error.
[carApp/src/components/ui/Card.tsx] — Widened style/contentStyle from bare ViewStyle to StyleProp<ViewStyle>; callers were already passing style arrays (which Card itself splats into arrays internally), and a bare ViewStyle rejects them.
[carApp/src/components/auth/VehicleForm.tsx] — handleBlur now validates the committed draft value from signUpDraft instead of e.nativeEvent.text. React Native 0.83 retyped TextInput onBlur as BlurEvent (NativeSyntheticEvent<TargetedEvent>, `target` only) and deprecated TextInputFocusEventData, so reading `text` off the blur event no longer typechecks. The native payload does still carry `text` (TextInputEventEmitter.cpp sets it on every text-input event, blur included), so this was a typing fix and not a runtime bugfix — the old handler was not validating undefined. Covered by src/components/auth/__tests__/VehicleForm.test.tsx.
[carApp/src/lib/supabase/auth.ts] — signInWithOtp/verifyOtp narrow OtpTarget with `target.email !== undefined` instead of truthiness. An empty-string email is falsy but still matches the email union member, so the truthy check never narrowed and left phone possibly-undefined against Supabase's credential types. Both now route through resolveOtpTarget, which rejects an empty or whitespace-only contact with 'Email is required'/'Phone number is required' instead of round-tripping to Supabase, and forwards the trimmed value. OtpTarget admits `{ email: '' }`, and the non-empty guarantee previously lived only in the two screens (otp-entry validates, otp-verify guards on `!contact`) — this keeps a future caller from bypassing it.
[carApp/src/types/navigation.ts] — Moved OtpVerifyParams here from the screen, as a type alias. useLocalSearchParams<T>() constrains T to Record<string, string | string[]>, and interfaces get no implicit index signature — so an interface silently fell through to the route-string overload and errored.
[carApp/tsconfig.json] — Added an exclude list for the Deno edge-function runtime files (supabase/functions/*/index.ts plus _shared/fcm.ts and _shared/email.ts). They use Deno globals and remote URL imports the app compiler cannot resolve, and accounted for 59 of the repo's tsc errors. _shared/webhookSignature.ts and all five function test suites remain typechecked.
[carApp/app/(provider)/vetting.tsx] — The vetting hub no longer dead-ends when a provider account has no provider_profiles row. It now separates a failed lookup (error → "could not load") from a genuinely missing row: for a provider/both account it creates the row via insertProviderProfile({ user_id }) (the trg_create_provider_vetting trigger seeds provider_vetting off it) and continues into the six steps, same as More → Provider "Start application". A pure-customer account still gets the "could not find your provider application" error. This closes the loop where a pure-provider signup whose profile insert failed was parked on pending-approval and "Continue your application" led to an unrecoverable error screen. Covered by app/(provider)/__tests__/vetting.test.tsx.
[carApp/app/(provider)/vetting.tsx] — Added a "Go to dashboard" CTA under the approved banner (sets activeMode 'provider', router.replace to /(provider-tabs)/jobs). The hub is the root of the (provider) stack whenever it is opened directly (pending-approval → Continue, a reload, a deep link), so there is no header back button, and the root gate's `if (inProviderGroup) return;` deliberately never pulls anyone out of the group — an approved provider was stranded on their own application with no exit. The CTA renders only while verification_status is 'approved'.
[carApp/src/components/provider/ServiceMenuEditor.tsx] — Fixed the price unit mismatch: the editor treated service_packages.base_price as cents (read `/100`, rendered centsToDisplay(base_price), wrote displayToCents) while the column is NUMERIC(10,2) whole dollars and the entire customer path reads it as dollars (bookingDraft.ts, search/provider/[id].tsx, search/book/[providerId].tsx all do `base_price * 100`). Effect was a 100x error in both directions — seeded $69 services showed as $0.69 in the provider's own menu, and a service added at $69 wrote 6900 and reached customers as $6,900.00 with the deposit charged against it. Now converts at the edges via priceCents(): dollars in, cents for display/validation, `cents / 100` dollars back out on save (parsing through cents keeps "69.99" off a float artifact). Note the booking JSONB service snapshot is a separate thing and does store cents — that side is unchanged. Covered by src/components/provider/__tests__/ServiceMenuEditor.test.tsx.
