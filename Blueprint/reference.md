...
[carApp/src/components/auth/VehicleForm.tsx] — handleBlur now validates the committed draft value from signUpDraft instead of e.nativeEvent.text. React Native 0.83 changed TextInput onBlur to deliver a BlurEvent whose nativeEvent carries no `text`, so the old handler was validating undefined on every blur. Covered by src/components/auth/__tests__/VehicleForm.test.tsx.
